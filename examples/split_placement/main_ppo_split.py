...
from verl.utils.reward_score import gsm8k, math, multiply, countdown, rlla_token_call_token #, rlla_call, rlla_token_param
...
def _select_rm_score_fn(data_source):
    ...
    elif data_source in ['rlla_token', 'rlla_token_call_token', 'rlla_call_token', 'rlla_semantic_call_semantic', 'rlla_call_semantic']:
        return rlla_token_call_token.compute_score
    # elif data_source == 'rlla_token_param':
    #     return rlla_token_param.compute_score
    # elif data_source == 'rlla_call':
    #     return rlla_call.compute_score
    # elif "rlla_token" in data_source:
    #     return rlla_token.compute_score
    # elif "rlla_call" in data_source:
...
class RewardManager:
    ...
    def __call__(self, data: DataProto, return_dict: bool = False, step: int = 0):
        ...
        for i in range(len(data)):
            ...
            #     sequences_str = "".join([message.get("content") for message in messages if message.get("role") == "assistant"])
            ...
            score = compute_score_fn(solution_str=sequences_str, ground_truth=ground_truth, step=step)
            reward_tensor[i, valid_response_length - 1] = score
